...
class FilaDoadorMedulaOssea extends Model
{
    ...
    public function chamarDoadorFila(){
        ...
        if(($filaDoadorMedulaOssea->totalNaFila())>0){
            $dadosDoadorChamado = DB::select('SELECT registro_doacao_id,cpf,tipo_doacao FROM fila_doadores_medula_ossea inner join registros_doacoes on fila_doadores_medula_ossea.registro_doacao_id = registros_doacoes.id_registro_doacao inner join doadores on registros_doacoes.doador_id = doadores.id_doador where posicao = 1');
            $doador = new Doador;   
            ...
            $filaDoadorMedulaOssea->removerDoadorFila(1); //Deleta o primeiro da fila e atualiza as posições.
        } else {
    ...
    public function removerDoadorFila($posicao){
        DB::delete('delete from fila_doadores_medula_ossea where posicao = ?',[$posicao]);   //Deleta da fila o doador de determinada posição
        DB::update('Update fila_doadores_medula_ossea set posicao = posicao-1 where posicao > ?',[$posicao]);  //Ajusta as posições: 3º vai pra segundo, 2º vai pra primeiro, e assim vai...
    }
